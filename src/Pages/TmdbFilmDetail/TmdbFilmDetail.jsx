...
function TmdbFilmDetail() {
        ...
        const fetchMovieDetail = async () => {
            try {
                ...
                const results = await Promise.allSettled([
                    fetch(`https://api.themoviedb.org/3/movie/${tmdbFilmId}?api_key=${api_key}`).then(res => res.json()),
                    fetch(`https://api.themoviedb.org/3/movie/${tmdbFilmId}/videos?api_key=${api_key}`).then(res => res.json()),
                    fetch(`https://api.themoviedb.org/3/movie/${tmdbFilmId}/reviews?api_key=${api_key}`).then(res => res.json())
                ]);

                const movieDetail = results[0].status === "fulfilled" ? results[0].value : null;
                const movieVideos = results[1].status === "fulfilled" ? results[1].value.results : [];
                const movieReviews = results[2].status === "fulfilled" ? results[2].value.results : [];

                if (!movieDetail) showNotification("error", "Getting a movie detail is errored!");
                if (!movieVideos.length) showNotification("info", "No videos available for this movie");
    ...
    const fetchTmdbFilmDetail = async () => {
        try {
            const res = await fetch(`${website_base_url}/api/tmdb-films/get?tmdbId=${tmdbFilmId}`, {
                method: 'GET',
                credentials: 'include'
            })
    ...
    if (!movieData || !movieData.movieDetail) {
        return (
            <div className="min-h-screen bg-gray-900 flex items-center justify-center">
                <p className="text-gray-300 text-xl">Sorry! This resource is not found!</p>
            </div>
        )
    }
                        ...
                        <div className="w-full md:w-2/3 lg:w-3/4">
                            <div className="flex flex-col space-y-4">
                                ...
                                >
                                    ...
                                    <p className="text-gray-300 leading-relaxed">{movieData.movieDetail.overview}</p>
                                </motion.div>
                                ...
                                {movieData.movieDetail.budget > 0 && (
                                    ...
                                    >
                                        ...
                                        <p className="text-gray-300">${movieData.movieDetail.budget.toLocaleString()}</p>
                                    </motion.div>
                                )}
                            </div>
                        </div>
                                        ...
                                        >
                                            <div className="flex items-start gap-4">
                                                <div
                                                    ...
                                                    style={{ backgroundColor: randomAvatarColor[Math.floor(Math.random() * randomAvatarColor.length)] }}
                                                >
                                                ...
                                                <div className="flex-1">
                                                    <h3 className="text-lg font-semibold text-gray-300">{review.author}</h3>
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="mt-4 text-center">
                                <h3 className="text-white text-xl font-medium">{selectedVideo.name}</h3>
                            </div>
                        </motion.div>
                    </motion.div>
                )}
