...
const Notification = ({ id, type, message, order, onClose, redirectUrl }) => {
  ...
  return (
    ...
    >
      <p className='w-full h-full text-center align-middle'>{type === 'error' ? 'Error: ' : ''}{message}</p>
      {redirectUrl && (
        <Link to={`/watch-detail/hot-movies${redirectUrl}`} className="inline-flex items-center align-middle">
