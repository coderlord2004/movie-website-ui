...
export default function NotificationIcon() {
    ...
    const deleteUserNotification = async (notificationId) => {
        ...
        try {
            await fetch(`${website_base_url}/api/user-notification/delete?notificationId=${notificationId}`, {
                method: 'DELETE',
                credentials: 'include'
            })
    ...
    const clearAllNotification = async () => {
        setLoading(true)
        try {
            const res = await fetch(`${website_base_url}/api/user-notification/clear-all`, {
                method: 'DELETE',
                credentials: 'include'
            })
    ...
    useEffect(() => {
        const getAllUserNotification = async () => {
            setLoading(true)
            try {
                const res = await fetch(`${website_base_url}/api/user-notification/get-all`, {
                    method: 'GET',
                    credentials: 'include'
                })
                const data = await res.json()
        ...
        >
            ...
            >
                ...
                {notifications.length !== 0 && (
                    <div className='w-[8px] h-[8px] bg-red-500 rounded-[50%] absolute top-[5px] right-[5px]'>
                    </div>
                )}
            </div>
                ...
                {isShowNotification && (
                    ...
                    >
                        {loading ? (
                            ...
                            notifications.length > 0 ? (
                                <>
                                    <div className='w-full text-purple-500 underline text-right cursor-pointer' onClick={clearAllNotification}>
                                        Clear all
                                    </div>
                                    ...
                                    {notifications.map((notification) => (
                                        <div className="w-full p-[5px] border-[2px] border-solid border-gray-700 rounded-[8px] flex justify-center items-start gap-[7px]">
                                            <div>
                                                <p className='border-b border-slate-500 text-red-400'>{notification.title}</p>
                                                <p>
                                                    {notification.description}{' '}
                                                    <Link to={`/watch-detail/hot-movies${notification.actionUrl}`} className="inline-flex items-center align-middle">
