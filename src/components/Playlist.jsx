...
export default function Playlist({ systemFilm, tmdbFilm }) {
    ...
    const handleCreationPlaylist = (e) => {
        ...
        fetch(`${website_base_url}/api/playlist/create-playlist?playlistName=${formData.get('playlistName')}`, {
            method: 'POST',
            credentials: 'include'
        })
            .then(res => res.json())
        ...
        <div className="w-full h-full relative rounded-[10px] flex flex-col gap-y-[12px] overflow-hidden">
            ...
            <div className='w-full flex flex-col justify-between items-center px-[20px]'>
                ...
                <div className="w-full flex flex-col">
                    {systemFilm && systemFilm.map((playlist, idx) => (
                        <div key={idx} className='w-full'>
                            <div className='flex justify-start items-center gap-x-[15px]'>
                                ...
                                <Tooltip text="Play film!" position="top">
                                    <div className="cursor-pointer" onClick={() => handlePlayPlaylist(playlist.systemFilms)}>
                                        <FaRegCirclePlay style={{ width: '25px', height: '25px' }} />
                                    </div>
                                </Tooltip>
                            </div>
                            <div className='w-full flex flex-wrap gap-[7px]'>
                                {playlist.systemFilms.map(film => (
                                    <div key={film.systemFilmId} className='min-w-[calc(100%/3-7px)] h-[180px] animate-skeleton rounded-[8px]'>
            ...
            <div className='w-full flex flex-col justify-between items-center px-[20px]'>
                ...
                <div className="w-full flex flex-col">
                    {tmdbFilm && tmdbFilm.map((playlist, idx) => (
                        <div key={idx} className='w-full'>
                            <div className='flex justify-start items-center gap-x-[15px]'>
                                <h1 className='text-[120%] font-bold py-[10px]'>{playlist.playlistName}</h1>
                            </div>
                            <div className='w-full flex flex-wrap gap-[7px]'>
                                {playlist.tmdbFilms.map(film => (
                                    <div key={film.id} className='min-w-[calc(100%/3-7px)] h-[180px] animate-skeleton rounded-[8px]'>
                                        <Image
                                            id={film.tmdbId}
                                            belongTo='TMDB_FILM'
                                        />
