                ...
                <tbody>
                    {allUserData.length ? allUserData.map((user, index) => (
                        <tr key={index} className="border-t border-gray-700 hover:bg-[#333333] transition-all duration-200">
                            ...
                            <td className="py-3 px-4 truncate max-w-[150px]" title={user.username}>{user.username}</td>
                            <td className="py-3 px-4 truncate max-w-[200px]" title={user.email}>{user.email}</td>
                            <td className="py-3 px-4 text-center truncate">{user.dateOfBirth}</td>
                            <td className="py-3 px-4 text-center truncate">{user.createdAt}</td>
                            <td className="py-3 px-4 text-center truncate">{user.ipAddress}</td>
                            <td className="py-3 px-4" title={user.country}>
                                <div className='flex justify-center items-center gap-2'>
                                    <span className="uppercase">{user.country}</span>
                                    <img src={`https://flagcdn.com/w40/${user.country && user.country.toLowerCase()}.png`} alt="Country Flag" className="w-6 h-4" />
                                </div>
                            </td>
                        ...
                        <tr>
                            <td colSpan="8" className="py-6 text-center text-gray-400">No user data available.</td>
                        </tr>
                    )}
                </tbody>
            </table>
