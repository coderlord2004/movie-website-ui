...
function Video({ key, videoData, onFormatDate }) {
    ...
    return (
        <div key={key} className="w-full h-[100px] rounded-[10px] border border-[1px] border-solid border-black flex p-[5px]">
            <div className="w-[50%] h-full bg-slate-500 rounded-[10px] mr-[5px] relative">
                ...
                {isPlaying ? (
                    ...
                    <div className="relative bg-slate-600" onClick={() => setPlaying(true)}>
                        ...
                        />
                        <div className={styles.playIcon + ' absolute top-[50%] left-[50%]'}></div>
                    </div>
                )}
