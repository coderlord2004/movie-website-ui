...
function Movie({ id, movieData, type, typeDetail, belongTo }) {
        ...
        >
            <div className={`relative overflow-hidden w-full h-auto`}>
                <div className={`w-full aspect-[2/3] rounded-[10px] animate-skeleton`}>
                    <img
                        src={belongTo === 'TMDB_FILM' ? `${image_base_url}/original/${movieData.poster_path}` : `${movieData.posterPath}`}
                        alt="No movie!"
                        onLoad={() => {
                            setLoading(false)
                        }}
                ...
                >
                    <Link
                        to={belongTo === 'TMDB_FILM' ? `/watch-detail/${type}/${typeDetail}/${movieData.id}/` : `/watch-detail/${movieData.systemFilmId}/`}
            ...
            <div className={`flex px-[3px] mt-[4px] justify-between items-start mb-[13px]`}>
                <p className={`w-[80%] mr-[6px] bg-transparent`}>{movieData.title || movieData.name}
                </p>
