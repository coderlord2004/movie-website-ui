...
export default function Comment({ systemFilmData, onUpdateData }) {
    ...
    useEffect(() => {
        const fetchComments = async () => {
            try {
                const res = await fetch(`${website_base_url}/api/comment/film/${systemFilmId}/comment-list`, {
                    method: 'GET',
                    credentials: 'include'
                })
    ...
    const handleEditComment = async (e, commentId, filmId, isReply = false) => {
        e.preventDefault();
        try {
            const res = await fetch(`${website_base_url}/api/comment/update-comment`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                credentials: 'include',
                body: JSON.stringify({
    ...
    const handleDeleteMyComment = async (commentId, filmId, isReply = false, parentId = null) => {
        try {
            const res = await fetch(`${website_base_url}/api/comment/delete-comment?commentId=${commentId}&filmId=${filmId}`, {
                method: 'POST',
                credentials: 'include'
            })
    ...
    return (
        ...
        >
            <div className="flex items-center gap-[10px] text-2xl font-bold text-white mb-6">
                ...
                <form onSubmit={(e) => handleCommentFilm(e, null)} className="flex flex-1 justify-between items-center gap-x-[12px]">
                    <input
                        type="text"
                        name='comment'
                    ...
                    />
                    <button type='submit' className='h-full hover:text-[lightblue] transition duration-200 ease-in-out cursor-pointer flex items-center justify-center'>
            ...
            <div className="space-y-6">
                {comments.length ? comments.map(comment => (
                    <div key={comment.commentId} className="bg-gray-800 rounded-lg p-4 relative">
                        ...
                        {editingCommentId === comment.commentId ? (
                            <form onSubmit={(e) => handleEditComment(e, comment.commentId, systemFilmId)}>
                                <input
                                    type="text"
                                ...
                                />
                                <button type="submit" className="text-green-400 text-sm mt-1">Save</button>
                            </form>
                        ) : (
                                    ...
                                    <div key={child.commentId} className="flex space-x-4">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            {editingReplyId === child.commentId ? (
                                                <form onSubmit={(e) => handleEditComment(e, child.commentId, systemFilmId, true)}>
                                                    <input
                                                        type="text"
                                            ...
                                            {child.userId === authUser.id && (
                                                <div className='flex gap-x-2 mt-1 text-[18px] text-gray-400'>
                                                    ...
                                                    <button onClick={() =>
                                                        handleDeleteMyComment(child.commentId, systemFilmId, true, comment.commentId)
                        ...
                        {replyingTo === comment.commentId ? (
                            <form onSubmit={(e) => handleCommentFilm(e, comment.commentId)} className="ml-10 mt-2">
                                <input
                                    type="text"
                                    name="comment"
                                ...
                                />
                                <button type="submit" className="text-blue-400 text-sm mt-1">Reply</button>
                            </form>
                        ) : (
                            <button onClick={() => setReplyingTo(comment.commentId)} className="text-blue-300 text-xs ml-10 mt-2">
                                Reply
                            </button>
                        )}
                            ...
                            <div className='absolute top-[10px] right-[10px] flex gap-x-[5px] text-[25px] group cursor-pointer'>
                                ...
                                <div className='hidden gap-x-[5px] group-hover:flex transition duration-200 ease-in-out absolute top-[-125%] right-0 bg-gray-600 rounded-lg p-[5px]'>
                                    ...
                                    <div
                                        ...
                                        onClick={() => handleDeleteMyComment(comment.commentId, systemFilmId)}
                                    >
                                        <MdDelete />
                                    </div>
                                </div>
